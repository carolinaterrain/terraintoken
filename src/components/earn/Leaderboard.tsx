...
const Leaderboard = () => {
  ...
  const { data: weeklyLeaders, isLoading: weeklyLoading } = useQuery({
  ...
  return (
    <section className="py-16 px-4 bg-gradient-to-b from-background/50 to-background">
      <div className="container mx-auto max-w-4xl">
        ...
        <Tabs value={timeframe} onValueChange={(v) => setTimeframe(v as "all" | "week")}>
          ...
          <TabsContent value="week">
            <GlassCard className="p-6">
              {weeklyLoading ? (
                ...
                <div className="space-y-3">
                  {weeklyLeaders.map((leader) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="text-2xl w-12 text-center">
                          {getRankBadge(leader.weekly_rank)}
                        </div>
                        <div>
                          <div className="font-display font-bold">
                            {truncateWallet(leader.user_wallet_address)}
                          ...
                          <div className="font-body text-sm text-muted-foreground">
                            {leader.weekly_uploads} uploads this week
                          </div>
                        </div>
                      </div>
                      <div className="font-display text-xl font-bold text-primary">
                        {Number(leader.weekly_trn_earned).toFixed(0)} TRN
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="text-center py-8 text-muted-foreground">
                  No contributions yet this week. Be the first!
                </div>
              )}
            </GlassCard>
          </TabsContent>
